...
### 💬 Chat System
- Session-based chat for all ephemeral users
- Presence-based authentication (no external OAuth required)
- Rate limiting (1 message per 2 seconds)
- Real-time message updates (5-second polling)
- Profanity filtering and message validation

**Note**: Discord OAuth was removed in favor of simpler session-based chat. Discord endpoints (`/api/auth/discord/*`) now return 410 Gone status with cookie cleanup. This change will be in effect for one release cycle to clean up existing `oauth_state` cookies before full removal.
