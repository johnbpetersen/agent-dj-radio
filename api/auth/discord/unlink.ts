...
async function discordUnlinkHandler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (presenceError || !presence) {
      logger.warn('Session not found for Discord unlink', { correlationId, sessionId })
    ...
    logger.error('Discord unlink unhandled error', { correlationId }, err)
    throw httpError.internal('Failed to unlink Discord account')
  }
}
