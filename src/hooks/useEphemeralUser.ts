...
export function useEphemeralUser(): UseEphemeralUserReturn {
  ...
  useEffect(() => {
    const initializeSession = async () => {
      try {
        // Check if ephemeral users are enabled by testing an endpoint
        const testResponse = await fetch('/api/users/active', {
          method: 'HEAD' // Just check if endpoint exists
        })
        
        if (testResponse.status === 404) {
          // Feature not enabled, fall back to existing user system or disable
