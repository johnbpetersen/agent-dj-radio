...
// ----------------------------
// Mock state hook (remains unchanged)
// ----------------------------
function useRoomState() {
  return {
    ...
    listeners: Array.from({ length: 14 }).map((_, i) => ({
      id: `u${i + 1}`,
      name: `User ${i + 1}`,
    })),
    djs: [
      { id: "dj1", name: "You", isCurrent: true },
      { id: "dj2", name: "Mo", isCurrent: false },
    ],
  };
}
...
// ----------------------------
// Utility helpers (remains unchanged)
// ----------------------------
...
function AudienceStrip({ listeners }: { listeners: { id: string, name: string }[] }) {
...
function CollapsiblePanel({ listeners }: { listeners: { id: string, name: string }[] }) {
  ...
  return (
    <div className="absolute top-1/2 -translate-y-1/2 left-0 z-20">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-64 h-[50vh] bg-black/40 backdrop-blur-sm rounded-r-lg shadow-2xl flex flex-col"
          >
            <h3 className="text-sm font-semibold text-white/80 p-4 border-b border-white/10 flex items-center">
              <Users className="w-4 h-4 mr-2" /> In The Room ({listeners.length})
            </h3>
            <div className="flex-1 overflow-y-auto p-2">
              {listeners.map(l => (
                <div key={l.id} className="text-white/70 text-sm px-2 py-1.5 rounded-md hover:bg-white/10">
                  {l.name}
                </div>
              ))}
            </div>
      ...
      <button
        ...
        className="absolute top-1/2 -translate-y-1/2 -right-8 w-8 h-16 bg-black/40 backdrop-blur-sm rounded-r-lg text-white/50 hover:bg-black/60 hover:text-white flex items-center justify-center"
      >
...
function ReactionBar() {
    return (
        <div className="flex items-center justify-center gap-4">
            <button className="px-4 py-1.5 bg-green-500/20 text-green-300 text-sm font-semibold rounded-full border border-green-500/30 hover:bg-green-500/30">
                <Heart className="w-4 h-4 inline-block mr-1" /> Awesome
            </button>
            <button className="px-4 py-1.5 bg-red-500/20 text-red-300 text-sm font-semibold rounded-full border border-red-500/30 hover:bg-red-500/30">
                <ThumbsDown className="w-4 h-4 inline-block mr-1" /> Lame
            </button>
        </div>
    );
}

function DJBooth({ nowPlaying, djs }: { nowPlaying: any, djs: any[] }) {
        ...
        <div className="relative w-full max-w-2xl">
            ...
            <div className="bg-gradient-to-b from-zinc-800 to-zinc-900 border-2 border-black rounded-t-xl shadow-2xl pt-20 pb-4 px-4">
                <div className="font-digital bg-black/50 border border-black rounded-md p-3 text-orange-400 shadow-[0_0_10px_rgba(251,146,60,0.4)]">
                    <div className="text-center text-xl tracking-wider mb-2 flex items-center justify-center">
                        <Radio size={16} className="mr-2 animate-pulse text-orange-500" /> {nowPlaying.title} - {nowPlaying.artist}
                    </div>
                    <div className="relative">
...
function ChatPanel() {
  ...
  return (
    <div className="absolute top-1/2 -translate-y-1/2 right-0 z-20">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ x: "100%" }}
            transition={{ duration: 0.3, ease: "easeInOut" }}
            className="w-72 h-[50vh] bg-black/40 backdrop-blur-sm rounded-l-lg shadow-2xl flex flex-col"
          >
            <h3 className="text-sm font-semibold text-white/80 p-4 border-b border-white/10 flex items-center">
              <MessageCircle className="w-4 h-4 mr-2" /> Chat
            </h3>
            <div className="flex-1 p-4 text-center text-white/40">
      ...
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="absolute top-1/2 -translate-y-1/2 -left-8 w-8 h-16 bg-black/40 backdrop-blur-sm rounded-l-lg text-white/50 hover:bg-black/60 hover:text-white flex items-center justify-center"
      >
...
export default function RoomScene() {
  const { nowPlaying, listeners, djs } = useRoomState();
